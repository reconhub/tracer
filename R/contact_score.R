...
#'
#' @param onset A vector of integers indicating dates of onset of the cases
#'     causing exposure.
#'
...
#' }
contact_score <- function(onset, R, lambda, w) {
    ...
    if (length(onset) < 1) {
        stop("'onset' must have at least one value.")
    }
    if (any(!is.finite(onset))) {
        stop("All values in 'onset' need to be finite, non-NA numbers.")
    }
    if (!is.finite(R)) {
